function tex_bode_pgfplot(ins,outs;folder=".",label="F:bode")
...
for i=1:nin  ## For each input
	...
	s*="\\begin{tikzpicture}\n"
  s*="\\begin{semilogxaxis}[height=2in,width=5in,"
  ...
  s*="legend style={at={(1.0,1.03)},anchor=south east},legend columns=1,legend cell align=left,cycle list name=linestyles*]\n"
	for j=1:nout  ## For each output
		s*="\\addplot+[black,line width=1pt,mark=none] table[x=frequency,y=m$((i-1)*nout+j)]{$folder/bode.out};\n"
	...
	for j=1:nout
		s*="\\addplot+[black,line width=1pt,mark=none] table[x=frequency,y=p$((i-1)*nout+j)]{$folder/bode.out};\n"
	end
	s*="\\end{semilogxaxis}\n"
	s*="\\end{tikzpicture}\n"
	s*="\\end{footnotesize}\n"
