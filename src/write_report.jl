function write_report(
    ...
    write_report([system], 0, [result], verbose; ss, bode, title, folder, filename, build, clean)
end
...
function write_report(
    ...
    rprt = "\\title{\nEoM Analysis\\\\\n$title\\\\\n}\n"
    rprt *= "\\author{Bruce P Minaker PhD PEng}\n"
    ...
    if length(vpts) > 1
        rprt *= tex_sstf_pgfplot(systems; ss)
        rprt *= tex_eig_pgfplot()
        rprt *= tex_bode_pgfplot(systems, vpts; bode)
    else
